...
fun List<Streak>.getCurrentStreak(today: LocalDate): Streak? = firstOrNull {
    val yesterdayWasInStreak = it.contains(today.minus(DatePeriod(days = 1)))
    if (yesterdayWasInStreak) {
        true
    } else {
        it.contains(today)
    }
...
fun List<Streak>.getLongestStreak(): Streak? = maxByOrNull { it.getDurationInDays() }
